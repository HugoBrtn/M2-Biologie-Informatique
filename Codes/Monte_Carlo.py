...
def MCsearch(phi, c, hp, T=220):
    ...
    Returns:
        tuple: (final_conformation, final_energy, best_conformation, best_energy, total_changes)
    """
    n = len(c)
    c_mini = c.copy()
    ...
    for i in range(phi):
        c_prime = c.copy()
        k = random.randint(1, len(c)-1)  # Choose a random residue (1-based index)
...
def REMCSimulation(phi, c, hp, E_star, T_init=160, T_final=220, chi=5):
    ...
    Args:
        ...
        c (pd.DataFrame): Initial conformation as a DataFrame with columns 'x' and 'y'.
        hp (str): HP sequence (Example: "HPPHHPH").
...
hp = "HPPHPPHPPH"
c = pd.DataFrame({
    'x': [2, 2, 2, 2, 1, 1, 0, 0, -1, -1],
    'y': [-2, -1, 0, 1, 1, 2, 2, 1, 1, 0]
})
...
plot_molecules_side_by_side(c, cp[2], hp)
print('Énergie:', cp[3])
print('Total des changements:', cp[4])
