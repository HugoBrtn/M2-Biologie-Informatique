...
def plot_molecule(c, hp_sequence, point_size=200, grid_color='gray', bg_color='white'):
    ...
    min_x, min_y = c['x'].min() - 1, c['y'].min() - 1
    max_x, max_y = c['x'].max() + 1, c['y'].max() + 1
    ax.set_xlim(min_x, max_x)
    ax.set_ylim(min_y, max_y)
    # Plot each residue
    for i in range(len(c)):
        x, y = c.iloc[i]['x'], c.iloc[i]['y']
    ...
    for i in range(len(c) - 1):
        x1, y1 = c.iloc[i]['x'], c.iloc[i]['y']
        x2, y2 = c.iloc[i+1]['x'], c.iloc[i+1]['y']
...
def plot_molecules_side_by_side(c1, c2, hp_sequence, point_size=200, grid_color='gray', bg_color='white',
    ...
    Args:
        c1 (pd.DataFrame): DataFrame with columns 'x' and 'y' for the first configuration.
        c2 (pd.DataFrame): DataFrame with columns 'x' and 'y' for the second configuration.
...
# ----- Test -----
#c = pd.DataFrame({
#    'x': [0, 1, 2, 3, 4, 4, 3, 2, 1, 0, 0, 1, 2, 3, 4, 4, 3, 2, 1, 0],
